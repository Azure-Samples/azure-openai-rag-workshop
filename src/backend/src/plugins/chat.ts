...
import { type Message, MessageBuilder, ChatResponse, ChatResponseChunk } from '../lib/index.js';
...
export class ChatService  {
  ...
  async run(messages: Message[]): Promise<ChatResponse> {
    const { completionRequest, dataPoints, thoughts } = await this.baseRun(messages);
    const chatCompletion = await this.chatClient.completions.create(completionRequest);
  ...
  async *runWithStreaming(
    ...
    const { completionRequest, dataPoints, thoughts } = await this.baseRun(messages);
    const chatCompletion = await this.chatClient.completions.create({
      ...completionRequest,
      stream: true,
    });
    let id = 0;
  ...
  private async baseRun(messages: Message[]) {
